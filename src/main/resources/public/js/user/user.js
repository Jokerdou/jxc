layui.use(['table','layer'],function(){
    ...
    table.on('toolbar(users)', function (data) {
        ...
        if (data.event == "add") { // 添加用户
            ...
            // 获取被选中的数据的信息
            var checkStatus = table.checkStatus(data.config.userId);
    ...
    function deleteUsers(data) {
        ...
        var BatchUser = table.checkStatus("userTable");

        var userDate = BatchUser.data
        ...
        layer.confirm('您确定要删除选中的记录吗？',{icon:3, title:'用户管理'}, function (index) {
            ...
            for(var i = 0; i < userDate.length; i++) {
                if(i < userDate.length - 1) {
                    ids = ids + userDate[i].userId + "&ids="
                } else {
                    ids = ids + userDate[i].userId;
                }
            }
            // console.log(ids);
    ...
    table.on('tool(users)', function (data) {
        ...
        if (data.event == "edit") { // 更新用户
            ...
            // 打开添加/修改用户的对话框
            openAddOrUpdateUserDialog(data.data.userId);

        } else if (data.event == "del") { // 删除用户
            // 删除单条用户记录
            deleteUser(data.data.userId);
        }
